...
// Configure your Cloudinary keys
cloudinary.config({
...
app.post("/api/setpfp-upload", async (req, res) => {
  try {
    ...
    if (!req.files || !req.files.pfp) return res.json({ success: false, message: "No file uploaded" });

    // 2. Auth: Check if token is valid
    const userLookup = await pool.query("SELECT username FROM users WHERE token=$1", [token]);
    if (userLookup.rows.length === 0) return res.json({ success: false, message: "Invalid token" });

    const username = userLookup.rows[0].username;
    const file = req.files.pfp; // express-fileupload syntax
    ...
    const newPfpUrl = uploadResult.secure_url;

    // 4. Update Database
    // We update BEFORE sending the response
    await pool.query("UPDATE users SET pfplink=$1 WHERE username=$2", [newPfpUrl, username]);

    // 5. Update Sockets (Real-time update)
    for (const [sId, s] of io.sockets.sockets) {
      // Assuming you store user info in the socket object
    ...
    // 6. Send Response
    res.json({ success: true, pfplink: newPfpUrl });
...
app.post('/api/register', authLimiter, async (req, res) => {
  ...
  try {
    // Multer populates req.body for custom uploads; express.json() populates it for stock PFPs.
    const { username, password } = req.body;
    let pfpUrl = "/images/stock-pfp/uzi-pfp.png"
    ...
    await pool.query(
      ...
      [username, passwordhash, pfpUrl, 'user', rawToken]
    );
    
    return res.json({ success: true, user: { username, pfplink: pfpUrl, role: 'user', token: rawToken } });
...
io.on('connection', (socket) => {
  ...
  socket.on('upload custom pfp', async (data)=> {
      ...
      socket.emit('system message', { text: 'No file uploaded.' });
      return;
    }
      const uploadResult = await new Promise((resolve, reject) => {
      cloudinary.uploader.upload_stream({
    ...
    for (const [sId, s] of io.sockets.sockets) {
      ...
      socket.emit('system message', { text: 'Failed to upload image to Cloudinary.' });
      return;
    }
  })
        ...
        if (commandName === 'kick') {
          ...
          for (const [sId, s] of io.sockets.sockets) {
            if (s.user && s.user.username === target) {
              ...
              const tokenToDelete = s.handshake.auth?.token;
              if (tokenToDelete) activeSessionCache.delete(tokenToDelete)
              break;
            }
          }
          return;
