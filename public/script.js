...
function showOnPage() {
    const allPfpCanvas = document.createElement("div");
       ...
       customPfp.onload = function(e) {
    const img = document.createElement('img');
    img.src = e.target.result; // This 'e' comes from the FileReader onload
    img.alt = 'image preview';
    img.style.maxWidth = '200px';
    img.style.margin = '10px';
    img.style.display = 'inline-block';
    img.style.cursor = 'pointer'; // Good practice to show it's clickable

    // Attach the click listener directly to the img
    img.addEventListener('click', (event) => {
        // Use 'event.target.src' because it's the clicked image
        morePfp.src = event.target.src; 
    ...
    customPfpPreview.innerHTML = ''; // Clear previous preview
...
customPfp.addEventListener('change', function(event) {
    ...
    if (file.type.startsWith('image/')) { 
        ...
        reader.onload = function(e) {
            // Check if preview container exists before appending
            if (!customPfpPreview) return;
            
            // Clear previous preview if you want to replace it
            customPfpPreview.innerHTML = ''; 
...
function setMutedUI(isMuted) {
    const input = document.getElementById("messageInput");
    const sendButton = document.querySelector(".send-message-button");
    if (input) input.disabled = isMuted;
    if (sendButton) sendButton.disabled = isMuted;
    if (input) input.placeholder = isMuted ? "You are muted" : "Type here";
}
...
socket.on("disconnect", (reason) => {
  ...
  socket.on("connect_error", (err) => {
    ...
    // if auth fails, clear token so next reload doesn't loop
...
socket.on("system message", (data) => {
  ...
  if (messages) {
    const msgDiv = document.createElement("div");
    msgDiv.className = "system-message";
    msgDiv.textContent = data?.text || "System";
...
socket.on("muted", (data) => {
  ...
  if (messages) {
    const msgDiv = document.createElement("div");
    msgDiv.className = "system-message";
    msgDiv.textContent = data?.text || "You have been muted";
    messages.appendChild(msgDiv);
  }
});
...
socket.on("unmuted", (data) => {
  ...
  if (messages) {
    const msgDiv = document.createElement("div");
    msgDiv.className = "system-message";
    msgDiv.textContent = data?.text || "You have been unmuted";
    messages.appendChild(msgDiv);
  }
});
...
socket.on("changed role", (data) => {
  ...
  if (data?.value) {
    ...
    const roleText = document.getElementById("role-text");
    if (roleText) roleText.textContent = "Role: " + data.value;
  }
});
...
socket.on("update pfp", (data) => {
  if (data?.value) {
    localStorage.setItem("pfplink", data.value);
    const accountPfp = document.querySelector(".account-pfp");
    if (accountPfp) accountPfp.src = data.value;
  }
});
...
socket.on("delete message", (data) => {
  const messages = document.querySelectorAll(".message");
  messages.forEach((message) => {
    if (message.dataset.id === data.messageId) message.remove();
  });
});

function requestHistoryWithRetry(retries = 2, delay = 700) {
  let received = false;
...
socket.on("kicked user", () => alert("You have been kicked!"));
 ...
 requestHistoryWithRetry();

  // ensure chat UI exists, then request history
...
async function saveUserData() {
  ...
  const customPfp = document.getElementById("customPfp");
  const file = customPfp.files[0]
...
function showGlobalChat(data) {
    ...
    interactiveStuffDiv.id = "interactiveStuffDiv";
    ...
    const changePfpLabel = document.createElement("div");
    changePfpLabel.textContent = "";
    changePfpLabel.className = "change-pfp-label";
    ...
    const changePfpButton = document.createElement("button");
    changePfpButton.className = "change-pfp-button";
    changePfpButton.textContent = "Change Profile Picture";
